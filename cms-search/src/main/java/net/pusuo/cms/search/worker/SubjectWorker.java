 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class SubjectWorker extends IndexWorker {
    
    private static final String SUBJECT_TOTAL_COUNT = 
        "select count(*) " +
        "from cms_subject s " +
        "join cms_entity e " +
    ...
    private static final String SUBJECT_MAX_ID = 
        "select max(entity_id) from cms_subject";
    
    private static final String SUBJECT_MIN_ID = 
        "select min(entity_id) from cms_subject";
    
    private static final String SUBJECT_TO_ID = 
        "select max(entity_id) " +
        "from (" +
        	"select entity_id from cms_subject where entity_id>? order by entity_id" +
        	") " +
        "where rownum<=?";
    
    private static final String SUBJECT_BLOCK_DATA = 
    	"select s.entity_id,s.subject_subtype,e.entity_name,e.entity_desc,e.entity_type," +
    	...
    	"join cms_entity e " +
    	"on s.entity_id=e.entity_id " +
    	"where s.entity_id>? and s.entity_id<=? " +
    ...
    public SubjectWorker() {
        ...
        lastIndexedPropertyName = "search.lastIndexedSubject";
    ...
    public int getTotalCount(int fromId) {
        ...
        try {
            ...
            pst = conn.prepareStatement(SUBJECT_TOTAL_COUNT);
            pst.setInt(1, fromId);
            rs = pst.executeQuery();
            rs.next();
    ...
    public int getMaxId() {
        ...
        try {
            conn = DbConnectionManager.getConnection();
            pst = conn.prepareStatement(SUBJECT_MAX_ID);
            rs = pst.executeQuery();
            rs.next();
            maxId = rs.getInt(1);
        }
    ...
    public int getMinId() {
        ...
        try {
            conn = DbConnectionManager.getConnection();
            pst = conn.prepareStatement(SUBJECT_MIN_ID);
            rs = pst.executeQuery();
            rs.next();
            minId = rs.getInt(1);
        }
    ...
    public List getBlockData(int fromId) {
        ...
        try {
            ...
            pst = conn.prepareStatement(SUBJECT_TO_ID);
            pst.setInt(1, fromId);
            ...
            pst = conn.prepareStatement(SUBJECT_BLOCK_DATA);
            pst.setInt(1, fromId);
            pst.setInt(2, toId);
            rs = pst.executeQuery();
            while (rs.next()) {
                ...
                entry.setSubtype(rs.getInt("subject_subtype"));
                entry.setName(rs.getString("entity_name"));
