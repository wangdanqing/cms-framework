...
public interface ChannelDao {
	...
	int insertBean(@BindBean Channel channel);

	@SqlQuery("select id, name, dir from channel where dir = :dir")
	@MapResultAsBean
	Channel findByName(@Bind("dir") String dir);

	@SqlQuery("select id, name, dir from channel where id = :id")
	@MapResultAsBean
	Channel getById(@Bind("id") long id);

	@SqlQuery("select id, name, dir from channel where id > :id order by id desc")
	List<Channel> query(@Bind("id") long id);
