...
//    private ConnectionPool connectionPool61 = null;
     ...
     * ��ݴ�������ʼ����Ӧ�����ӳ�
     */
    public DbConnectionDefaultPool(String num) {
        //this.manager = manager;
        props = new Properties();
        propDescriptions = new Properties();
        //Initialize all property values
        initializeProperties(num);
        //Load any existing property values
        loadProperties(num);
    }
    /**
     ...
     * Returns a database connection.
     */
    public Connection getConnection(String num) {
        if (connectionPool == null) {
            //block until the init has been done
            synchronized(initLock) {
                //if still null, something has gone wrong
    ...
    /**
     * ����ָ�������ӳ�
     * @see com.hexun.cms.client.xport.db.DbConnectionProvider#start()
     */
    protected void start(String conn) {
        //acquire lock so that no connections can be returned.
    	conn = conn.trim();
        synchronized (initLock) {
            //Get properties
            String driver = props.getProperty("driver"+conn);
            String server = props.getProperty("server"+conn);
            String username = props.getProperty("username"+conn);
            String password = props.getProperty("password"+conn);
	    ...
	    //System.out.println(password);
            ...
            double connectionTimeout = 0.0;
            try {
                minConnections = Integer.parseInt(props.getProperty("minConnections"+conn));
                maxConnections = Integer.parseInt(props.getProperty("maxConnections"+conn));
                connectionTimeout = Double.parseDouble(props.getProperty("connectionTimeout"+conn));
		//log.info("=============");
		//log.info("driver:"+driver+"||server:"+server+"|| username:"+username+"||password:"+password);
		//log.info("=============");
            }
            catch (Exception e) {
                System.err.println("Error: could not parse default pool properties. " +
            ...
            String logPath = props.getProperty("logPath"+conn);

            try {
		//System.out.println("=====start:" + "driver:"+driver+"||server:"+server+"|| username:"+username+"||password:"+password);
                connectionPool = new ConnectionPool(driver, server, username, password,
                    minConnections, maxConnections, logPath, connectionTimeout);
            }
            catch (IOException ioe) {
     ...
     */
    private void initializeProperties(String num) {
    	if(num!=null && !num.equals(""))
    	{
	        props.put("driver"+num,"");
	        props.put("server"+num,"");
	        props.put("username"+num,"");
	        props.put("password"+num,"");
	        props.put("minConnections"+num,"");
	        props.put("maxConnections"+num,"");
	        props.put("logPath"+num,"");
	        props.put("connectionTimeout"+num,"");
	
	        propDescriptions.put("driver"+num,"JDBC driver. e.g. 'oracle.jdbc.driver.OracleDriver'");
	        propDescriptions.put("server"+num,"JDBC connect string. e.g. 'jdbc:oracle:thin:@203.92.21.109:1526:orcl'");
	        propDescriptions.put("username"+num,"Database username. e.g. 'Scott'");
	        propDescriptions.put("password"+num,"Database password. e.g. 'Tiger'");
	        propDescriptions.put("minConnections"+num,"Minimum # of connections to start with in pool. Three is the recommended minimum");
	        propDescriptions.put("maxConnections"+num,"Maximum # of connections in dynamic pool. Fifteen should give good performance for an average load.");
	        propDescriptions.put("logPath"+num,"Absolute path name for log file. e.g. 'c:\\logs\\jiveDbLog.log'");
	        propDescriptions.put("connectionTimeout"+num,"Time in days between connection resets. e.g. '.5'");
    	}else
    	{
            props.put("driver","");
            props.put("server","");
     ...
     */
    private void loadProperties(String target) {
    	if(target!=null && !target.equals("") && target.length()>0)
    	{
	        String driver = PropertyManager.getProperty("DbConnectionDefaultPool.driver"+target);
	        String server = PropertyManager.getProperty("DbConnectionDefaultPool.server"+target);
	        String username = PropertyManager.getProperty("DbConnectionDefaultPool.username"+target);
	        String password = PropertyManager.getProperty("DbConnectionDefaultPool.password"+target);
	        String minConnections = PropertyManager.getProperty("DbConnectionDefaultPool.minConnections"+target);
	        String maxConnections = PropertyManager.getProperty("DbConnectionDefaultPool.maxConnections"+target);
	        String logPath = PropertyManager.getProperty("DbConnectionDefaultPool.logPath"+target);
	        String connectionTimeout = PropertyManager.getProperty("DbConnectionDefaultPool.connectionTimeout"+target);
	
	        if (driver != null) {  props.setProperty("driver"+target, driver);  }
	        if (server != null) {  props.setProperty("server"+target, server);  }
	        if (username != null) {  props.setProperty("username"+target, username);  }
	        if (password != null) {  props.setProperty("password"+target, password);  }
	        if (minConnections != null) {  props.setProperty("minConnections"+target, minConnections);  }
	        if (maxConnections != null) {  props.setProperty("maxConnections"+target, maxConnections);  }
	        if (logPath != null) {  props.setProperty("logPath"+target, logPath);  }
	        if (connectionTimeout != null) {  props.setProperty("connectionTimeout"+target, connectionTimeout);  }
    ...
    private class ConnectionPool implements Runnable {
         ...
         * @exception  java.sql.SQLException if connections were in use after
         * <code>millis</code>.
         */
    ...
    public class ConnectionWrapper  implements Connection {
        ...
        public CallableStatement prepareCall(String param1,int param2,int param3,int param4) throws SQLException {
            return connection.prepareCall(param1,param2,param3,param4);
        }
